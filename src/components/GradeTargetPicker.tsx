    ...
    <div style={{
      ...
      <div className="frow">
        <div className="fgroup">
          ...
          <select className="finput" value={value.class_id ?? ''} style={{ cursor: 'pointer' }}
            ...
            <option value="">Sin clase (solo historial)</option>
      ...
      {value.class_id && (
        <div className="fgroup" style={{ marginBottom: 0 }}>
          <label className="flabel">Categoría del cuaderno</label>
          {cats.length === 0 ? (
            <p style={{ fontSize: 12.5, color: 'var(--warn)', lineHeight: 1.5, margin: 0 }}>
              Esta asignatura todavía no tiene categorías en el cuaderno. Crea una
              (Exámenes, Trabajos…) y vuelve aquí; mientras tanto la evaluación se
              guardará solo en el historial.
            </p>
          ) : (
            <select className="finput" value={value.category_id ?? ''} style={{ cursor: 'pointer' }}
              ...
              <option value="">Solo el historial, no el cuaderno</option>
      ...
      {value.class_id && value.category_id && (
        <p style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 10, lineHeight: 1.5 }}>
          Cada alumno que evalúes aparecerá al momento en el cuaderno, en una columna
          propia de este instrumento. <strong>Si vuelves a evaluar al mismo alumno, su
          nota anterior se sustituye.</strong>
        </p>
      )}
    </div>
  );
