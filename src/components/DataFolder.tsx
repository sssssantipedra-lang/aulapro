...
export function DataFolder({ profileId, courseLabel, onClearSchoolYear }: Props) {
  ...
  async function makeBackup() {
    ...
    setBusy(false);
    refresh();
    toast(file ? '✅ Copia de seguridad creada' : 'No hay datos que copiar todavía');
  }
  ...
  async function restore(file: string) {
    ...
    setBusy(false);
    if (ok) {
      toast('✅ Copia restaurada. Se recarga la aplicación…');
  ...
  return (
    <div className="card" style={{ gridColumn: '1 / -1' }}>
      <div className="card-hd">
        <div className="card-ttl"><HardDrive size={14} color="var(--accent-d)" />Mis datos</div>
        {info && (
      ...
      {desktop && (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 18 }}>
          ...
          <button className="btn-ghost" onClick={makeBackup} disabled={busy}>
            <Archive size={14} />Crear copia ahora
          </button>
        </div>
      )}
      ...
      {desktop && info && info.backups.length > 0 && (
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
            Copias de seguridad ({info.backups.length})
          </div>
            ...
            {info.backups.map(b => (
              <div key={b.file} style={{
                ...
                <span style={{ flex: 1, color: 'var(--text-2)', minWidth: 0 }}>
                  {new Date(b.at).toLocaleString('es-ES', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
          ...
          <button
            ...
            onClick={async () => {
              ...
              refresh();
              toast('Copias de seguridad borradas');
            }}
          >
            <Trash2 size={12} />Borrar todas las copias
          </button>
        </div>
      )}
      ...
      <div style={{ paddingTop: 16, borderTop: '0.5px solid var(--border)' }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--text)', marginBottom: 6 }}>
          Final de curso {courseLabel && `· ${courseLabel}`}
        </div>
        <p style={{ fontSize: 12.5, color: 'var(--text-2)', lineHeight: 1.6, marginBottom: 14 }}>
          Cuando termine la tercera evaluación puedes vaciar el curso: se borran clases, alumnos,
          notas, evaluaciones, asistencia e informes. <strong>Se conservan tus rúbricas y dianas</strong>,
          que te servirán el año que viene.
          {desktop && ' Antes de borrar se guarda una copia de seguridad.'}
        </p>
        ...
        {confirmClear ? (
          <div style={{ background: 'rgba(239,68,68,0.07)', border: '0.5px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '13px 15px' }}>
            <p style={{ fontSize: 12.5, color: 'var(--text-2)', lineHeight: 1.55, marginBottom: 12 }}>
              ¿Seguro? Se borrará el trabajo de este curso. Podrás recuperarlo desde las copias
              de seguridad de arriba si te arrepientes.
            </p>
            <div style={{ display: 'flex', gap: 9 }}>
              <button
                ...
                onClick={async () => {
                  ...
                  refresh();
                  toast('✅ Curso vaciado. Tus rúbricas siguen ahí.');
                }}
              >
                Sí, vaciar el curso
              </button>
          ...
          <button className="btn-ghost" style={{ color: 'var(--danger)' }} onClick={() => setConfirmClear(true)}>
            <CalendarX size={14} />Vaciar el curso
          </button>
        )}
      </div>
    </div>
